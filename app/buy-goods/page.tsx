...
export default function BuyGoodsPage() {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (error?.message === "AMOUNT_MISMATCH") {
          console.error(
            `Amount mismatch! Expected: KES ${error.details.expected}, Provided: KES ${error.details.provided}`
          );
        } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header title="Buy Goods`" showBack />

      <main className="max-w-md mx-auto px-4 py-6 pb-24">
        {/* Context Section */}
        <div className="bg-purple-50 rounded-xl p-4 mb-6">
          <h2 className="font-semibold text-purple-900 mb-2">
            Pay at stores and shops
          </h2>
          <p className="text-purple-700 text-sm">
            Use the merchant's till number to pay for goods and services
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          {/* Warning */}
          <WarningMessage
            message="Payment to wrong Till number is non-refundable."
            type="info"
          />
          ...
          <Button
            ...
            fullWidth
            size="lg"
            className="bg-purple-600 hover:bg-purple-700 text-white py-4 text-lg font-semibold rounded-xl disabled:bg-gray-400"
